...
* 其实就是将字符串转为数字
* 怎么转？
    * 把一个单词的每一个字符对应的编码进行求和，把得出的结果作为下标
        * 弊端是下标重复的可能性高，出现后者覆盖前者的情况
    * 幂的连乘
        * 可以保证下标的唯一性
        * 但是某些单词可能会出现下标值过大的情况，造成内存空间的浪费，且创建出来的数组很多空间是没有使用的
...
* 有三种方法
    * 线性探测法
        * 每次探测步长为1
        * 会带来一个问题--聚集
        * 聚集--一连串的填充单元格，影响哈希表的性能
    * 二次探测
        * 每次探测步长为1, 4, 9, 16, ...
        * 用来解决线性探测带来的聚集问题
        * 多个关键字有相同下标时存在探测问题
    * 再哈希化
        * 对关键字再用一个哈希函数进行探测，这个新的哈希函数不同于第一次使用的哈希函数且结果不等于0，目的是在对步长做一次优化
        * 这个新的哈希函数可以用： stepSize = constant - (key % constant)
        * constant是一个质数，小于数组的容量
        * 例如：5 = 5 - (key % 5)
* 删除数据项时不能把删除的项赋值为null，否则会导致存在的元素找不到，可以赋值为一个特殊的值比如-1

**哈希化的效率**
* 哈希表中的插入和删除效率是非常高的
    * 如果没有发生冲突，效率会更高
    * 如果发生了冲突，那么效率就会降低，取决于探测的长度
* 平均探测长度和平均存取时间取决于填充因子，随着填充因子的变大，其探测长度也变大
* 什么是填充因子?
    * 表示当前哈希表中已知的数据项和当前哈希表的长度的比值
    * 装填因子 = 总数据项 / 哈希表的长度
    * 开放地址法的装填因子最大是1，因为他必须寻找到空白的单元才能将元素插入
    * 链地址法的装填因子可以大于1，因为拉链法可以无限的延伸下去，只要你愿意，但是越到后面效率越低
